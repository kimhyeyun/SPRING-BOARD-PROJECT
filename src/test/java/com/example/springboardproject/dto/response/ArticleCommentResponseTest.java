...
class ArticleCommentResponseTest {
    ...
    @DisplayName("자식 댓글이 없는 게시글 + 댓글 dto를 api 응답으로 반환할 때, 댓글 시간 내림차순 + ID 오름차순으로 정리한다.")
    @Test
    ...
    @Test
    void givenArticleWithCommentsDto_whenMapping_thenOrgarnizesParentAndChildCommentsWithCertainOrders() {
        // Given
        LocalDateTime now = LocalDateTime.now();
        Set<ArticleCommentDto> articleCommentDtos = Set.of(
                createArticleCommentDto(1L, null, now),
